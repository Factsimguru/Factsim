...
class FactSignal(object):
    ...
    def merge(self, othersignal, tick):
        ...
        for i in list(dictsignal.keys()):
            if i in list(othersignal.keys()):
                dictsignal[i] = dictsignal[i] + othersignal[i]
        for i in list(othersignal.keys()):
            if i not in list(dictsignal.keys()):
...
class Network(object):
    ...
    def getmembers(self, activedict):
        for i in list(self.memberlist.keys()):
    ...
    def runFactsim(self):
        for i in list(self.members['IN'].values()):
...
class ConnEntity(object):
    ...
    def getFactsimInput(self):
        for nw in list(self.NW_In.values()):
            if nw != 0:
...
class Decider(ConnEntity):
    ...
    def __init__(self, entity):
        ...
        self.output_signal = ('','')
        self.comparator = '>'
        if 'control_behavior' in list(self.Entity.properties.keys()):
            self.controlbehavior = parsecontrolbeh(self.Entity.properties['control_behavior'])
            for i in list(self.controlbehavior['decider_conditions'].keys()):
    ...
    def buildotputsignal(self):
        ...
        if self.first_signal[1] == 'signal-anything':
            ...
            elif self.second_signal[1] in list(signalin.keys()):
                secargument = signalin[self.second_signal[1]]
            else:
                secargument = 0
            for i in list(signalin.keys()):
                if self.compare(signalin[i], secargument):
                    if self.output_signal[1] == 'signal-everything':
                        if self.ccfInput:
                            ...
                            for k in list(signalin.keys()):
                                self.FSignalOUT.merge({k: 1}, self.tick)
                            return
                    elif self.output_signal[1] in list(signalin.keys()):
                        if self.ccfInput:
        ...
        elif self.first_signal[1] in list(signalin.keys()):
...
class Arithmetic(ConnEntity):
    ...
    def __init__(self, entity):
        ...
        if 'control_behavior' in list(self.Entity.properties.keys()):
            self.controlbehavior = parsecontrolbeh(self.Entity.properties['control_behavior'])
            for i in list(self.controlbehavior['arithmetic_conditions'].keys()):
                if i == 'constant':
...
class Constant(ConnEntity):
    ...
    def __init__(self, entity):
        ...
        if 'control_behavior' in list(self.Entity.properties.keys()):
            ...
            for i in list(self.controlbehavior.keys()):
...
class Lamp(ConnEntity):
    ...
    def __init__(self, entity):
        ...
        if 'control_behavior' in list(self.Entity.properties.keys()):
            ...
            if 'use_colors' in list(self.controlbehavior.keys()):
                self.controlbehavior['use_colors'] = eval(self.controlbehavior['use_colors'].capitalize())
            for i in list(self.controlbehavior['circuit_condition'].keys()):
                if i == 'constant':
...
class Factsimcmd(object):
    ...
    def openbpr(self):
        ...
        self.root.withdraw()
        self.bprfilename = tkinter.filedialog.askopenfilename()
        self.bprintfile = open(self.bprfilename, encoding='utf-8')
        ...
        self.bprintdata = self.bprintdata.decode('utf-8')
        ...
        for i in self.parsed_entities:
            ...
            for j in entitydata:
                ...
                currentEntity.proporder += list(dictprop(j).keys())
        ...
        for pole in list(self.poledict.values()):
            ...
            if len(conn) > 0:
                ...
                for i in list(groups.values()):
                    valuelist += i
                if pole.Entity.id not in list(groups.keys()) and pole.Entity.id not in valuelist:
    ...
    def printNetworks(self):
        for i in list(self.Networks.keys()):
            print('{} Networks'.format(i))
            for j in list(self.Networks[i].keys()):
                ...
                print('            members In  {}'.format([member.Entity.id for member in
                                                                    list(self.Networks[i][j].members.get('IN').values())]))
                print('            members Out {}'.format([member.Entity.id for member in
                                                                    list(self.Networks[i][j].members.get('OUT').values())]))
    ...
    def runsim(self, nticks):
        ...
        while self.globaltick <= end:
            ...
            for i in list(self.activedict.values()):
    ...
    def printresults(self, t=('', ''), e=True, n=False):
        ...
        if e:
            print('########################################\n'+\
                  ...
                  '########################################\n')
            print('|{:40}'.format('OUTPUT'), end=' ')
            w = 50
            for time in range(t[0], t[1]):
                dt = '{:04d}'.format(time)
                print('|'+'{!s:^{w}}'.format(dt,w=w), end=' ')
            print('|')
            for ent in self.activeEntities:
                print('|{:40}'.format(ent.name), end=' ')
                for time in range(t[0], t[1]):
                    print('|'+'{!s:^{w}}'.format(ent.getFactsimOutput(time), w=w), end=' ')
                print('|')
            ...
            for ent in self.activeEntities:
                ...
                if not onlyOUT and ent.name is not None:
                    ...
                    for time in range(t[0], t[1]-1):
                        print('|'+'{!s:^{w}}'.format(ent.FSignalIN.ontick(time), w=w), end=' ')
                    print('|')
        ...
        if n:
            ...
            for time in range(t[0], t[1]):
                ...
                print('|'+'{!s:^{w}}'.format(dt,w=w), end=' ')
            print('|')
            for color in list(self.Networks.keys()):
                for nw in list(self.Networks[color].values()):
                    aux = '|Factsim Network        N{:04d}'.format(nw.id)
                    print(aux + '{}'.format(' '*(41-len(aux))), end=' ')
                    for time in range(t[0], t[1]):
                        print('|'+'{!s:^{w}}'.format(nw.getFactsimOutput(time), w=w), end=' ')
                    print('|')
    ...
    def writeresults(self, t=('', ''), e=True, n=True, efilters=(), nfilters = ()):
        ...
        if e:
            print('########################################\n'+\
                  'Entities activity during simulation\n'+\
                  '########################################\n', file=rsf)
            print('|{:40}'.format('OUTPUT'), end=' ', file=rsf)
            w = 50
            for time in range(t[0], t[1]):
                dt = '{:04d}'.format(time)
                print('|'+'{!s:^{w}}'.format(dt,w=w), end=' ', file=rsf)
            print('|', file=rsf)

            for ent in self.activeEntities:
                print('|{:40}'.format(ent.name), end=' ', file=rsf)
                for time in range(t[0], t[1]):
                    print('|'+'{!s:^{w}}'.format(ent.getFactsimOutput(time), w=w), end=' ', file=rsf)
                print('|', file=rsf)

            print('---------------------------------------------------------------', file=rsf)
            print('|{:40}'.format('INPUT'), end=' ', file=rsf)
            for time in range(t[0], t[1]):
                dt = '{:04d}'.format(time)
                print('|'+'{!s:^{w}}'.format(dt, w=w), end=' ', file=rsf)
            print('|', file=rsf)
            for ent in self.activeEntities:
                ...
                if not onlyOUT and ent.name is not None:
                    print('|{:40}'.format(ent.name), end=' ', file=rsf)
                    for time in range(t[0], t[1]-1):
                        print('|'+'{!s:^{w}}'.format(ent.FSignalIN.ontick(time), w=w), end=' ', file=rsf)
                    print('|', file=rsf)
        ...
        if n:
            ...
            for color in list(self.Networks.keys()):
                for nw in list(self.Networks[color].values()):
                    aux = '|Factsim Network        N{:04d}'.format(nw.id)
                    print(aux + '{}'.format(' '*(41-len(aux))), end=' ', file=rsf)
                    for time in range(t[0], t[1]):
                        print('|'+'{!s:^{w}}'.format(nw.getFactsimOutput(time), w=w), end=' ', file=rsf)
                    print('|', file=rsf)
        rsf.close()
    ...
    def printreport(self):
        ...
        countpnw = 0
        out_of_tick = 0
        for i in list(self.Networks['red'].values()):
            if len(i.poles) > 0:
                countpnw += 1
            if i.tick < self.globaltick:
                out_of_tick += 1
        for i in list(self.Networks['green'].values()):
            if len(i.poles) > 0:
                countpnw += 1
            if i.tick < self.globaltick:
                out_of_tick += 1
        totalnw = len(list(self.Networks['red'].values())) + len(list(self.Networks['green'].values()))
        ...
        aritm = 0
        decid = 0
        for i in list(self.activedict.keys()):
    ...
    def getCNEntities(self):
        result = []
        for i in self.Entities:
            if 'connections' in list(i.properties.keys()):
                result += [i]
        return result
